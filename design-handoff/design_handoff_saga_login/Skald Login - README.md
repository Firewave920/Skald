...
- **Fields:** a vertical flex with **gap 24px**. Every field uses the same pattern:
  ...
  1. **"Where is your server?"** — a horizontal flex (`align-items: flex-end`, gap 12px):
     ...
     - **Host input** — `flex: 1`, the standard underline input. **Prefill `***REDACTED-HOST***:13378`**,
       placeholder `***REDACTED-HOST***:13378`, `spellcheck=false`.
...
- `host: string` (default `'***REDACTED-HOST***:13378'`)
...
- `login/tokens.jsx` — shared Skald tokens (`SKALD`), the `Wash` background, the `MiniTitlebar`, and the
  `PREFILL` defaults (`{ scheme: 'http', host: '***REDACTED-HOST***:13378', user: 'Testadmin' }`).
